package com.android.fresco.demo.widget;
...
public class OverScrollView extends FrameLayout implements OnTouchListener {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsBeingDragged) {
                    ...
                    if (isOverScrolled()) {
                        ...
                        float n = calculateMoveY(deltaY);
                        int k = (int)(deltaY * n);
                        Log.e("", "----ttttt------n = "+n+"------deltaY = "+deltaY+"------realY = "+k);
                        scrollBy(0, k);
                    } else {
                        scrollBy(0, deltaY);
                    }
    ...
    private float calculateMoveY(float y){
        float result = kaka / (Math.abs(y) + 2f * kaka);
        return result;
    }
